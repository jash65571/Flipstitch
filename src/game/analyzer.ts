 ...
 */
import { canonicalEdgeKey, oppositeSide, solveLevel, targetEdges } from "./solver.ts";
import type { Difficulty, Level, Side, StitchEdge } from "./types.ts";
...
export type LevelMetrics = {
  ...
  /** Decisions that sit on at least one full solution path — the choices a
   *  solving player actually faces. Comparable to the historic "meaningful
   *  decisions" column. */
  ...
  /** Distinct states from which no completion is reachable (stuck or doomed). */
  doomedStates: number;
  /** Fewest stitches played before the thread can first be doomed. */
  earliestDoomDepth: number | null;
  /** Distinct states where at least one legal stitch dooms the thread. */
  dangerousDecisions: number;
  /** Total stitches that lead (immediately or later) to a dead end. */
  unsafeMoveCount: number;
  /** Safe stitches available at dangerous-decision states. */
  safeAlternativeCount: number;
  /** Longest gap between a wrong choice and the discovery of the dead end.
   *  Also the lookahead a player needs to *avoid* the trap. */
...
export function analyzeLevel(level: Level): LevelMetrics {
  ...
  function visit(currentHole: string, side: Side, used: ReadonlySet<number>): StateInfo {
    ...
    let info: StateInfo;
    let childInfos: StateInfo[] = [];
    if (complete) {
      info = { depth, hasSolution: true, stuck: false, minStuckDepth: Number.POSITIVE_INFINITY };
    } else if (moves.length === 0) {
      info = { depth, hasSolution: false, stuck: true, minStuckDepth: depth };
      ...
      for (const edgeIndex of moves) {
        ...
        hasSolution = hasSolution || child.hasSolution;
        if (child.minStuckDepth < minStuckDepth) minStuckDepth = child.minStuckDepth;
      }
      info = { depth, hasSolution, stuck: false, minStuckDepth };
    }
    ...
    // A state with ≥2 moves where at least one child is doomed is a dangerous
    // decision: the player can strand the thread from here. Consequence depth
    // is how many stitches later the nearest dead end fires — the lookahead a
    // player needs to see the danger in advance.
    if (!complete && !info.stuck && moves.length >= 2) {
      let dangerous = false;
      for (const child of childInfos) {
        if (!child.hasSolution) {
          unsafeMoveCount += 1;
          dangerous = true;
          const consequence = child.minStuckDepth - depth;
          if (consequence > maxConsequenceDepth) maxConsequenceDepth = consequence;
        } else {
          safeAlternativeCount += 1;
        }
      }
      if (dangerous) dangerousDecisions.add(key);
    }
    return info;
  }

  visit(level.startHole, level.startSide, new Set());
  ...
  return {
    totalStitches: total,
    solutionCount: solveLevel(level).solutionCount,
...
export function measureLevel(level: Level): { metrics: LevelMetrics; score: DifficultyScore } {
